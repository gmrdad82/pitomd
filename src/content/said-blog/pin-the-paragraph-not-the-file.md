...
have ever attempted, the thinking arrived scattered. Measurements landed
in `garden/beds.md`. The plant wishlist grew in `garden/plants.md`. The
budget — the sad, honest budget — lived at the bottom of
`garden/money.md`, below two years of older sad honest budgets. Then came
the moment every project hits: I wrote the task "order the lumber" and
realized it depended on one paragraph in the first file, three lines in
the second, and a single row of numbers in the third. A link to a _file_
would have handed me `money.md` and wished me luck scrolling.
...
briefing travels as one block, sources labeled, nothing retyped. If a pin is
sitting one paragraph off, you slide its start and end line by line from
the keyboard until it grips exactly what you meant. And when you would
...
## Rewrite the note and the pins follow it

The objection is obvious, and it was mine first: notes change. I rewrite `plants.md` every time my ambition and my budget
renegotiate. In any normal system, line-level links rot the first time you
touch the file — the pin says lines 12 through 15, the paragraph has moved
to line 30, and the link now points at a recipe for compost.

Said and Done. keeps a snapshot of the text each pin was holding, and when
the file changes, it goes hunting. If your paragraph moved, the pin finds
it and quietly re-points itself at the new lines. If you reworded it but
it is recognizably the same thought, the pin re-grips there and refreshes
its snapshot. Only when the paragraph is genuinely gone does the pin let
...
There is no setup behind any of this. The map is simply what falls out of
pinning paragraphs instead of pasting filenames; the structure was
always in your project, and now something bothers to look. I check it
the way you glance at a weather chart before a weekend: not to act,
just to know where things are leaning. When a new file starts sprouting
edges, that is a corner of the project waking up; when an old hub goes
quiet, something got finished, or abandoned, and either way the map
noticed before I did.

## Nothing left to hold in my head

The lumber got ordered, the beds got built, and the dog supervised
throughout with the confidence of someone who has never read a budget.
What stayed with me was quieter than any feature: at no point in that
project did I hold the map in my head. Every task knew its paragraphs,
every paragraph stayed pinned through every rewrite, and the overview
knew the shape of the whole thing so I did not have to.
