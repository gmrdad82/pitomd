...
if [ -f "$INBOX" ] && grep -q '^## .*UNPROCESSED' "$INBOX"; then
  echo "BLOCKED by log law: docs/claude/INBOX.md still has UNPROCESSED owner message(s). Drain each into the active working plan (todos/decisions/discussion), then change its '⛔ UNPROCESSED' heading to '✅ processed' (or remove the block) before stopping." >&2
  exit 2
fi
exit 0
